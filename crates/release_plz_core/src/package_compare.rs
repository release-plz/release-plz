...
pub fn get_cargo_package_files(package: &Utf8Path) -> anyhow::Result<Vec<Utf8PathBuf>> {
    // If this crate was packaged already (either locally for git-only or downloaded from a registry), we can list
    // files directly from disk without invoking `cargo package`.
