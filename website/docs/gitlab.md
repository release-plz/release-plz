...
`release-plz` can also run in GitLab CI/CD, however the setup is slightly more complex than with the Github CI.

## 1. Generate a token for release-plz

`release-plz` authenticates with Gitlab via a token. This needs to be generated and added to the CI/CD variables:
...
3. Give the token the Maintainer role and grant the `api`, `read_api`, `read_repository`, and `write_repository` scopes
4. Configure this token as a masked variable named `RELEASE_PLZ_TOKEN` in Settings -> CI/CD -> Variables -> Add variable
5. Go to Manage -> Members and find the bot account that was created for this token and get the username
6. The username can be converted to the email address using this format: "$USERNAME@noreply.gitlab.com"
7. Configure this email address as a variable named `RELEASE_PLZ_BOT_EMAIL` in Settings -> CI?CD -> Variables -> Add variable

## 2. Put the component definition in a repository

The component definition below takes care of a lot of edgecases, and it's therefore recommended to put this in a separate repository and so that
the component can be used by all your Rust repositories.
...
spec:
  inputs:
    ...
    image:
      default: "marcoieni/release-plz"
      description: "The Docker image to run in, needs to have `release-plz` installed"
    cwd:
      type: string
      default: "."
...
.release-plz-defaults:
  ...
  before_script:
    ...
    - git checkout "$CI_COMIT_REF_NAME" $( if [ "$[[ inputs.git_submodule_recursive ]]" = "true" ]; then echo "--recurse-submodules"; fi )
...
By putting it at `templates/release-plz/template.yml` it can be used in your repositories in your `.gitlab-ci.yml` as:
```yaml
include:
...
If your repository contains submodules that are hosted on a different forge, for example Github, some things need to be configured.
This is only needed if the submodule is hosted elsewhere, Gitlab supports cloning submodules from Gitlab repositories.

1. Enable recursive cloning of submodules by setting `git_submodule_recursive` to `true`
2. Configure an access token for the Git host by setting `git_credentials` to an array of strings which conform to [the `.git-credentials` storage format](https://git-scm.com/docs/git-credential-store#_storage_format):
```yaml
include:
  - component: $CI_SERVER_FQDN/path/to/component/release-plz@branch_name
    inputs:
